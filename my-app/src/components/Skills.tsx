...
export default function Skills() {
    ...
    return (
        <>
            <div ref={containerRef} className="bg-[var(--background)] flex flex-col gap-5 pt-30 p-20 h-full items-center justify-between">
                <div className="w-fit h-fit flex gap-4 font-extrabold">
                    {text.split("").map((char, index) => (
                        <span key={index}
                            className=" skill-letter px-5 py-3 text-center text-[var(--color)] outline-2 outline-[var(--color)] rounded-md">
                            {char}
                        </span>
                    ))}
                </div>
                <span className="text-[var(--color)] opacity-70">
                    Here are the tools that I can use in web development for frontend and backend.
                </span>
                <div
                    className=" w-full h-full flex gap-5 mt-10">
                    {data.map((items, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className="skill w-1/3  p-5 bg-neutral-900 rounded-2xl flex flex-col gap-5"
                                ...
                                <div className="flex gap-3 items-center">
                                    <div className={`bg-gradient-to-br ${Color} w-15 h-15 p-[4px] rounded-full`}>
                                        <div className="icon-spin w-full h-full bg-[var(--background)] flex items-center justify-center rounded-full" >
                                            {Icon && <Icon className="w-full scale-100 text-[var(--color)]" />}
                                        </div>
                                    </div>
